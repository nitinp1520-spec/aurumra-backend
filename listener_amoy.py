...
def insert_tx(tx_hash, address, amount, raw, ts):
    conn = db()
    cur = conn.cursor()
    try:
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (tx_hash, address.lower(), "IN", amount, raw, "MATIC", ts))
        conn.commit()
    except:
        pass
    conn.close()
    ...
    while True:
        ...
        for (address,) in rows:
            ...
            for tx in incoming:
                ...
                ts = tx["block_signed_at"].replace("Z", "")
                
                insert_tx(tx_hash, address, amount, raw, ts)

                matic_price = get_matic_in_inr()
                send_notification(address, amount, amount * matic_price)

                print(f"✅ {amount:.6f} MATIC received for {address}")
